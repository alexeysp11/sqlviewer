...
namespace SqlViewer.Models.DbPreproc
{
    public class MssqlDbPreproc : IDbPreproc
    {
        ...
        public void DisplayTablesInDb()
        {
            ...
            try
            {
                string sqlRequest = MainVM.DataVM.MainDbClient.GetSqlRequest("Mssql\\TableInfo\\DisplayTablesInDb.sql"); 
        ...
        public void GetColumnsOfTable(string tableName)
        {
            ...
            try
            {
                string sqlRequest = string.Format(MainVM.DataVM.MainDbClient.GetSqlRequest("Mssql\\TableInfo\\GetColumns.sql"), tn[0], tn[1]); 
        ...
        public void GetForeignKeys(string tableName)
        {
            try
            {
                string sqlRequest = string.Format(MainVM.DataVM.MainDbClient.GetSqlRequest("Mssql\\TableInfo\\GetForeignKeys.sql"), tableName);;
        ...
        public void GetSqlDefinition(string tableName)
        {
            ...
            {
                string[] tn = tableName.Split('.');
                string sqlRequest = string.Format(MainVM.DataVM.MainDbClient.GetSqlRequest("Mssql\\TableInfo\\GetSqlDefinition.sql"), tn[0], tn[1]);
                DataTable dt = UserDbConnection.ExecuteSqlCommand(sqlRequest);
                if (dt.Rows.Count > 0) 
                {
