...
## 🛠️ Известные проблемы / Планируемые улучшения

В этом разделе описаны технические проблемы и архитектурные аспекты, выявленные на текущем этапе разработки. Эти моменты планируется включить в будущие итерации для повышения отказоустойчивости и масштабируемости системы.

### 1. Проблема «грохочущего стада» при опросе
В настоящее время несколько настольных клиентов могут синхронизировать свои запросы на опрос состояния, что потенциально может привести к значительным скачкам нагрузки на API Gateway и ETL Service («эффект грохочущего стада»).

* **Предлагаемое решение:** Реализовать **дрожание** (добавление небольшой случайной задержки к каждому запросу) для более равномерного распределения нагрузки во времени.

* **Обработка ошибок:** Реализовать **экспоненциальную задержку** для увеличения задержки между повторными попытками, если сервер возвращает ошибки 5xx или ответы с ограничением скорости (429).

### 2. Оптимизация ресурсов на стороне клиента
Текущий фоновый процесс для активных операций поддерживает постоянный интервал опроса независимо от состояния приложения.

* **Предлагаемое решение:** Реализовать адаптивную стратегию опроса. Интервал должен увеличиваться (например, с 2 до 30 секунд), если окно приложения **свернуто**, что позволит уменьшить ненужный сетевой трафик и использование ЦП как на стороне клиента, так и на стороне сервера.

### 3. Непрерывность распределенной трассировки (Jaeger)
Из-за пакетной обработки в ETL Worker и Data Transfer Worker контекст трассировки `CorrelationId` может иногда нарушаться при группировке сообщений.

* **Предлагаемое решение:** Вручную распространять `SpanContext`, извлекая заголовки трассировки из отдельных сообщений в пакете и создавая дочерние сегменты для каждой задачи обработки.
